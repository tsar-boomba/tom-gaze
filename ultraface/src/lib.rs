...
use nd::{s, ArrayView3, Axis};
use ndarray as nd;
use ort::{ExecutionProviderDispatch, InMemorySession, SessionOutputs, Value};
...
type Rect = common::Rect<f32>;
type Point = common::Point<f32>;
...
impl UltrafaceVariant {
    ...
    pub fn path(&self) -> &str {
        match self {
            ...
            UltrafaceVariant::W320H240Quantized => "assets/ultraface-RFB-320-quant.onnx",
        }
    }
}

/// Loaded Ultraface model, ready for inference with post-processing thresholds.
pub struct UltrafaceModel<'a> {
...
impl<'a> Debug for UltrafaceModel<'a> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("UltrafaceModel")
...
impl<'a> UltrafaceModel<'a> {
    ...
    /**
     * Input image should already be sized correctly according to the version of the model used
     */
    pub fn detect(&self, input: &[f32]) -> Result<Vec<(Rect, f32)>, Error> {
        assert!(input.len() == (self.width * self.height) as usize * 3);
    ...
    fn preproc(&self, input: &[f32]) -> nd::Array4<f32> {
        // TODO: resize image to height * width and put in dst
        let array = ArrayView3::from_shape((self.height as usize, self.width as usize, 3), input)
            .unwrap()
            .permuted_axes((2, 0, 1));
    ...
    fn postproc(&self, raw_nn_out: &SessionOutputs<'_>) -> Result<Vec<(Rect, f32)>, Error> {
        // Extract confidences
        ...
        let bboxes: Vec<Rect> = bboxes
            .chunks(4)
            .map(|bbox| {
                Rect::from_points(Point::new(bbox[0], bbox[1]), Point::new(bbox[2], bbox[3]))
            })
            .collect();
